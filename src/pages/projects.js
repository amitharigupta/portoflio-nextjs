    ...
    return (
        <article className="w-full flex items-center justify-between relative rounded-br-2xl rounded-3xl border border-solid border-dark bg-light shadow-2xl p-12 dark:bg-dark dark:border-light lg:flex-col lg:p-8 xs:rounded-2xl xs:rounded-br-3xl xs:p-4">
            ...
            <Link href={link} target={"_blank"} className="w-1/2 cursor-pointer overflow-hidden rounded-lg lg:w-full">
                <FramerImage src={img} alt={title} className={"w-full h-auto"} whileHover={{scale: 1.05}} transitiion={{duration: 0.2}} priority sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 50vw" />
            </Link>
            <div className="w-1/2 flex flex-col items-start justify-between pl-6 lg:w-full lg:pl-0 lg-pt-6">
                ...
                <div className="mt-2 flex items-center ">
                    ...
                    <Link href={link} className={"ml-4 rounded-lg bg-dark text-light p-2 px-6 text-lg font-semibold dark:bg-light dark:text-dark sm:px-4 sm:text-base"} target={"_blank"} >
                        Visit Project
                    </Link>
                </div>
            </div>
        ...
        <article className="w-full flex flex-col items-center justify-center rounded-2xl border border-solid border-dark bg-light p-6 relative dark:bg-dark dark:border-light xs:p-4">
            ...
            <div className="w-full flex flex-col items-start justify-between mt-4">
                ...
                <div className="w-full mt-2 flex items-center justify-between">
                    <Link href={link} className={"text-lg font-semibold underline md:text-base"} target={"_blank"} >
                        Visit
                    </Link>
